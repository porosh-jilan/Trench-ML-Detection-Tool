...
class DetectTrenchesCNN(object):
    ...
    def execute(self, parameters, messages):
        ...
        # v12.9.11: block-processing wrapper. For normal-size DEMs this is a
        # transparent pass-through (identical output); for very large DEMs it
        # tiles the raster into overlapping blocks so 32-bit ArcMap does not hit
        # a MemoryError. Threshold ~140 Mpx (below the size that OOMs).
        out_fc = trench_ml_core.detect_trenches_cnn_blocked(
            parameters[0].valueAsText,
            weights,
            norm,
            parameters[1].valueAsText,
